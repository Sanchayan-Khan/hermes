...
export default function Navigation() {
  ...
  const handleNavigation = (href: string) => {
    // if going to a protected page and not authenticated, go to /login
    const protectedPages = ["/journal", "/postcard", "/lores", "/profile"];
    if (!isAuthenticated && protectedPages.includes(href)) {
      router.push("/login");
      return;
    }
    if (pathname !== href) {
